#include "Config.h"

// v1.01: 0x1416E3A59
// v1.02: 0x1416E2BA9
bool* pvMode;

// v1.01: 0x140C8C688
// v1.02: 0x140C8B6C0
SIG_SCAN(sigPVMark, "pv_mark", "xxxxxxx");

// v1.01: 0x140CBDF50
// v1.02: 0x140CBCF20
SIG_SCAN(sigCopyright, "rom/copyright.farc", "xxxxxxxxxxxxxxxxxx");

// v1.01: 0x1402454C0
// v1.02: 0x140245350
SIG_SCAN
(
	sigLyrics,
	"\x40\x53\x48\x83\xEC\x20\x48\x8B\xD9\xE8\x00\x00\x00\x00\x80\xBB\x00\x00\x00\x00\x00",
	"xxxxxxxxxx????xx?????"
);

// v1.01: 0x1406F3E20
// v1.02: 0x1406F2820
SIG_SCAN
(
	sigGameMode,
	"",
	"xxxx?xxxxxxxxxxx?xxx????xxx????xxxxxxxxxxxxxx?xxx????x????xx????xxxxxxxxxxxxxxx?"
)

// v1.01: 0x14040B370
// v1.02: 0x14040B270
SIG_SCAN
(
	sigGetPVMode,
	"",
	"xxxx?xxxx?xxxx?xxxxxxxxxx?????xxxxxxxxxxxxx????xxxxxxxxxxxxxxxxxxxx?????"
)

// v1.01: 0x14065DE20
// v1.02: 0x14065C5B0
SIG_SCAN
(
	sigTaskPauseMenuSwitch,
	"\x48\x89\x5C\x24\x00\x48\x89\x74\x24\x00\x48\x89\x4C\x24\x00\x57\x48\x83\xEC\x30\x48\x8B\xF9\xE8\x00\x00\x00\x00\x90\x48\x8D\x05\x00\x00\x00\x00\x48\x89\x07\xC7\x87\x00\x00\x00\x00\x00\x00\x00\x00",
	"xxxx?xxxx?xxxx?xxxxxxxxx????xxxx????xxxxx????????"
)

HOOK(void, __fastcall, _SetGameMode, sigGameMode(), __int64 a1)
{
	original_SetGameMode(a1);

	WRITE_MEMORY((uint8_t*)sigLyrics() + 0x38, bool, *pvMode);
}

extern "C" __declspec(dllexport) void Init()
{
	Config::init();

	if (Config::pvMark)
	{
		WRITE_MEMORY(sigPVMark(), uint8_t, 0x00);
	}

	if (Config::copyrightMark)
	{
		WRITE_MEMORY(sigCopyright(), const char, "rom/disablewm.farc");
	}

	if (Config::hideLyrics)
	{
		WRITE_MEMORY((uint8_t*)sigTaskPauseMenuSwitch() - 0x66, bool, true);

		uint8_t* instrAddr = (uint8_t*)sigGetPVMode() - 0x10;
		pvMode = (bool*)(instrAddr + readUnalignedU32(instrAddr + 0x3) + 0x20);
		printf("[Disable Watermarks] pvMode: 0x%08x\n", pvMode);
		INSTALL_HOOK(_SetGameMode);
	}
}
